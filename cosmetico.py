...
def executar_jogo(dbz):
    ...
    screen = display.set_mode((1900, 900), 0, 32)
    delay = 150
    clock = time.Clock()
    ...
    while True:
        ...
        # Range gerado está dentro do mapa?
                if p[0] >= 0 and p[0] < dbz.tamanho and p[1] >= 0 and p[1] < dbz.tamanho:
                    ...
                    if dbz.mapa[p[0]][p[1]] == TERRENO_MONTANHA:
                        cor = montanha
                    elif dbz.mapa[p[0]][p[1]] == TERRENO_GRAMA:
                        cor = grama
...
# Verifica se irá desenhar a casa do mestre Kame
                    if p == dbz.casa_kame:
                        screen.blit(kame_house, rect.Rect((x * 36 + 5, y * 36), (36, 36)))

# Verifica se irá desenhar uma esfera do dragão
                    if p in dbz.esferas:
                        screen.blit(sphere, rect.Rect((x * 36 + 6, y * 36 + 8), (36, 36)))
